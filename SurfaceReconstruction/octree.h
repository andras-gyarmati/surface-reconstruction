...
public:
    ...
    void insert(file_loader::vertex& point_to_insert) {
        ...
        if (point_to_insert.position.x < m_top_left_front->x || point_to_insert.position.x > m_bottom_right_back->x || point_to_insert.position.y <
            m_top_left_front->y ||
            point_to_insert.position.y > m_bottom_right_back->y || point_to_insert.position.z < m_top_left_front->z || point_to_insert.position.z >
            m_bottom_right_back->z) {
            //std::cout << "Point is out of bounds." << " pos: " << glm::to_string(point_to_insert) <<
            //    " m_top_left_front: "
            //    << glm::to_string(*m_top_left_front) << " m_bottom_right_back: " << glm::to_string(*m_bottom_right_back)
        ...
        m_children[octant] = nullptr;
        if (octant == top_left_front) {
            m_children[octant] = new octree_vertex(glm::vec3(m_top_left_front->x, m_top_left_front->y, m_top_left_front->z), glm::vec3(mid.x, mid.y, mid.z));
        }
        else if (octant == top_right_front) {
            m_children[octant] = new octree_vertex(glm::vec3(mid.x, m_top_left_front->y, m_top_left_front->z), glm::vec3(m_bottom_right_back->x, mid.y, mid.z));
        }
        else if (octant == bottom_right_front) {
            m_children[octant] = new octree_vertex(glm::vec3(mid.x, mid.y, m_top_left_front->z), glm::vec3(m_bottom_right_back->x, m_bottom_right_back->y, mid.z));
        }
        else if (octant == bottom_left_front) {
            m_children[octant] = new octree_vertex(glm::vec3(m_top_left_front->x, mid.y, m_top_left_front->z), glm::vec3(mid.x, m_bottom_right_back->y, mid.z));
        }
        else if (octant == top_left_bottom) {
            m_children[octant] = new octree_vertex(glm::vec3(m_top_left_front->x, m_top_left_front->y, mid.z), glm::vec3(mid.x, mid.y, m_bottom_right_back->z));
        }
        else if (octant == top_right_bottom) {
            m_children[octant] = new octree_vertex(glm::vec3(mid.x, m_top_left_front->y, mid.z), glm::vec3(m_bottom_right_back->x, mid.y, m_bottom_right_back->z));
        }
        else if (octant == bottom_right_back) {
            m_children[octant] = new octree_vertex(glm::vec3(mid.x, mid.y, mid.z), glm::vec3(m_bottom_right_back->x, m_bottom_right_back->y, m_bottom_right_back->z));
        }
        else if (octant == bottom_left_back) {
            m_children[octant] = new octree_vertex(glm::vec3(m_top_left_front->x, mid.y, mid.z), glm::vec3(mid.x, m_bottom_right_back->y, m_bottom_right_back->z));
        }
        m_children[octant]->insert(already_stored_point);
        m_children[octant]->insert(point_to_insert);
        m_children[octant]->m_point = nullptr;
    ...
    bool find(const file_loader::vertex& pos) const {
        if (pos.position.x < m_top_left_front->x || pos.position.x > m_bottom_right_back->x || pos.position.y < m_top_left_front->y ||
            pos.position.y > m_bottom_right_back->y || pos.position.z < m_top_left_front->z || pos.position.z > m_bottom_right_back->z) {
            return false;
        }
    ...
    void knn_search(octree_vertex* node, const file_loader::vertex& target, int k, std::priority_queue<neighbor>& nearest) {
        ...
        int octant = octree_vertex::get_octant(target.position, (*node->m_top_left_front + *node->m_bottom_right_back) / 2.0f);
        knn_search(node->m_children[octant], target, k, nearest);

        for (int i = 0; i < 8; i++) {
            if (i != octant && node->m_children[i]) {
                float boundary_dist = glm::distance(target.position, (*node->m_top_left_front + *node->m_bottom_right_back) / 2.0f);
                if (nearest.size() < k || boundary_dist < nearest.top().distance) {
                    knn_search(node->m_children[i], target, k, nearest);
                }
            }
